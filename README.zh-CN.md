...
![发送前语气检查演示](docs/demo.gif)

_演示使用虚构文本，不包含个人信息。_

## 发送前语气检查是什么？

发送前语气检查是一款小型网页应用，适合在给同事、上级、老师、客户、朋友或家人写消息时使用。它可以检查礼貌程度、表达意图、是否期待回复、紧急程度、正式程度、情绪和反讽等信号。

应用使用 [TypeSafe Jev](https://docs.typesafe.ai/introduction) 对类型化问题进行判断，并返回结构化数值和概率分布。发送前语气检查把这些数值转成有针对性的沟通目标检查，但不会生成一段替代文本。
...
在 macOS 上，也可以双击 `start.command`。它会启动开发服务器，等待应用就绪，然后自动打开网页。使用发送前语气检查时请保持这个终端窗口开启；在其中按 Control-C 即可停止服务。
...
本仓库采用 [MIT 许可证](LICENSE)。公开源码也不代表本地 API 已适合公开托管；认证和用量限制需要单独处理。
